...
func TestDecodeEmptyCursorSuccess(t *testing.T) {
	...
	mockCursor.On("Close", testutils.MockArgs(1)...).Return(nil)
	mockCursor.On("Next", testutils.MockArgs(1)...).Return(false)
...
func TestDecodeCursorSuccess(t *testing.T) {
	mockCursor := new(mocks.MongoCursor)
	mockCursor.On("Close", testutils.MockArgs(1)...).Return(nil)
	mockCursor.On("Next", testutils.MockArgs(1)...).Return(true).Once()
	...
	mockCursor.On("Next", testutils.MockArgs(1)...).Return(false).Once()
	mockCursor.On("Err").Return(nil)
